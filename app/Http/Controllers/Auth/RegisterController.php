    ...
    public function confirmation(UserToken $token)
    {
        ...
        if($token->type == 'userConfirmation')
        {
            ...
            if(auth()->check())
             ...
             Session::flash('error',"Your email address has been successfully verified.");    
        }
        
        if($user->hasRole("educator")) return redirect()->route("auth.educator");
        if($user->hasRole("employer")) return redirect()->route("auth.employer");
    }
}
